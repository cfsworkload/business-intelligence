...
NoSQL DB and dashDB. Select your app from the Bluemix dashboard and select **Enviroment Variables** from the left sidebar. These variables 
contain the information needed to connect to each service. Some of these variables will be important later. Copy the following fields into a 
...
5. There is a visualization tab shown in the sidebar, but that isn't connected yet. We'll get to that soon.
...
**Launch** button in the top right. You can look at individual tables through the thing on the left (placeholder - put a picture in).
