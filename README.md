...
10. At the top of the page, underneath your app name, click the link in the **Routes** field to launch your app.
11. Take a moment to check out your Simple Data Pipes website! Then, create a new pipe from the left sidebar.
12. In the **Type** field, select **Salesforce**. Give it a **Name**, and also a **Description** if you wish, then click **Proceed**.
  ![EXAMPLE](images/Pipes Create New.png)

13. Copy the **Callback URL**. You'll need this later to connect Salesforce.
![EXAMPLE](images/Pipes Callback URL.png)

We've got all that we need from the Simple Data Pipes app at the moment, so let's head back to Bluemix.
...
4. Click **Skip** to sync the data now. In the future, you have the option to schedule regular updates from Salesforce.
5. Click **Run now** to start loading your data into your Pipes app.
...
## Load the report visualization into the website

Now that the data is connected to the report engine, you can create a visualization of the data.
...
5. Select the pipe and click **Visualization** in the left navigation bar. You'll see a visualization of the Salesforce
sample data! You can edit the report or make your own using the Embeddable Reporting service.
